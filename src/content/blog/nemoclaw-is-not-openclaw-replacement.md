...
description: "NemoClaw（尼莫）、OpenClaw（龍蝦）、NVIDIA（牙醫）——GTC 2026 後社群問「該換到 NemoClaw 嗎？」但 repo 第一行就寫了 plugin。從程式碼結構到實踐者踩坑紀錄，拆解這場被包裝成革命的外掛安裝。"
pubDate: 2026-03-19
category: "技術"
...
如果你看過《海底總動員》，你會記得那個場景：小丑魚尼莫被牙醫 Dr. P. Sherman 從大海撈進雪梨診所的魚缸裡。牙醫覺得自己在保護尼莫，但尼莫從來就不屬於魚缸。

OpenClaw 的 mascot 是一隻龍蝦。NemoClaw 的名字裡有 Nemo。而 NVIDIA——那個堅持要幫所有人建魚缸的角色——剛好是一位很有錢的牙醫。
...
門檻低、能力強、成本可控。這就是 32 萬顆星的底層邏輯。龍蝦在大海裡活得好好的。
...
Meta、Samsung、SK 明令禁止員工在公司裝置上安裝 OpenClaw。大海突然變得很危險，而牙醫準時出現了。
...
這就是為什麼「NemoClaw 是 OpenClaw 的上位版」是一個危險的敘事。它不是上位版，是外掛程式。而這個外掛程式的完整功能，綁在一條特定的硬體供應鏈上。

《海底總動員》的結局是尼莫回到了大海。現實世界裡，龍蝦可能沒這麼幸運——因為這位牙醫不只有魚缸，還有整條供應鏈。而且他不收掛號費，只收 GPU 的錢。
