...
打開銀行 app，看到帳戶餘額 57,000。但這個數字是錯的——月底要繳 15,500 的房租，信用卡帳單三萬出頭還沒繳，分期付款散落在國泰和玉山兩張卡裡，債務每月要還 3,000。真正能動的錢？可能是負的。
...
這份文件最後長到近 540 行，定義了六張資料表的欄位、可動用金額的計算公式、12 個測試案例、四個開發里程碑。但更重要的是它經歷了什麼——在 85 分鐘內跑了 6 輪審查。
...
第三次是在算旅遊預算。我問「5 月底去泰國能花多少」，AI 只預留了一張信用卡帳單的錢，忽略了整個六月的房租、網路費、電信費、債務還款。我貼了完整的六月收支時間軸，它才發現預留金額差了快兩萬。

這些錯的共同點：**AI 缺乏的不是計算能力，而是使用者的生活脈絡。** 它不知道你用現金繳卡費、不知道高鐵對你來說是偶發支出不是通勤工具、不知道你的提領習慣是一次領好一整個月的用量。

還有一個更深的設計錯誤——AI 把信用卡帳單的分期金額設計成動態計算，結果被自己寫的週期推進邏輯打敗，帳單金額憑空蒸發八千塊。那個故事太長，我[另外寫了一篇](/blog/cashflow-installment-snapshot)。

## 從逐次糾正到系統化防錯

同一個錯犯三次，就不是 AI 的問題了——是我沒有把知識結構化。

我建了三層防錯機制。最底層是 **CLAUDE.md**，專案設定檔，每次對話自動載入。寫進去的都是硬規則：「信用卡帳單以現金繳納，非自動扣款」「高鐵是一次性支出」「查詢可用金額時預設查到下一個收入日」。

往上一層是 **SKILL.md**，AI Skill 的行為規範。這裡放的是操作流程——Business Rules 禁止假設 payment_method，Playbooks 定義了「提領建議」的四步驟 SOP，還有預留計算必須展開完整月份時間軸的規定。比 CLAUDE.md 更結構化，也更長。
...
我用 Claude Code 的 Skill 系統寫了一個 cashflow skill。不需要 MCP server，不需要額外基礎設施。Skill 就是一份 Markdown，告訴 AI：「使用者說『我能花多少』的時候，打 `POST /planning/can-i-spend` 這支 API，回來的數字除以 100 轉成元，用這個格式顯示。」
...
我請了一組五人安全審計團隊（還是 subagent）。他們翻遍了所有檔案，標記了幾個風險：CLAUDE.md 裡有支付習慣和消費偏好、commit log 裡有具體的帳戶餘額和薪資數字、docs/ 裡有完整的設計文件包含真實的財務數據範例、data/ 裡有 SQLite 資料庫。

最後的做法是建立一個獨立的 `cashflow-release` repo，用 `.gitattributes` 的 `export-ignore` 排除敏感檔案。`publish.sh` 腳本負責自動化同步。上傳的是乾淨的程式碼，不帶任何個人資料。
...
要說不安的話，倒不是擔心這個專案沒辦法給別人用——它本來就是針對我自己的需求開發，沒顧及市場性和擴充性。真正讓我不安的是：原來我的財務狀況是如此悲觀。工具把數字攤開來，比自己模糊的感覺殘忍得多。

但這也是第一步。透過這個工具串接記帳軟體，逐漸掌控自己的資源——這一步，無論 AI agent 再怎麼強大，還是得靠慢慢調整的個人消費習慣。
